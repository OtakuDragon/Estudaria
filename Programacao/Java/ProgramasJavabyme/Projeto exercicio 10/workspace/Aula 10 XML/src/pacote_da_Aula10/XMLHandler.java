...
public class XMLHandler {
	...
	static Object read(Class<?> clazz) throws Exception{
		...
		for(int i=0;i<atribs.length;i++){
			...
			if(tipo.equals("int")){
				atribs[i].set(obj, Integer.parseInt(value));
			}else if(tipo.equals("String")){
				atribs[i].set(obj,value);
			}else if(tipo.equals("double")){
				atribs[i].set(obj, Double.parseDouble(value));
			}else{ 	atribs[i].set(obj, Boolean.parseBoolean(value));}
